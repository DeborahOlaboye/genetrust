...
(define-trait contract-registry-trait
    (
        ...
        ;; Returns (ok principal) or (err uint)
        (get-version 
            ((string-utf8 100) uint) 
            (response principal uint))

        ;; Register a new version for a named contract (admin-only).
        ;; Returns (ok version-number) or (err uint)
        (register-version 
            ((string-utf8 100) principal) 
            (response uint uint))
        ...
        (get-capabilities 
            ((string-utf8 100) uint) 
            (response (list 10 (string-utf8 50)) uint))
    )
)
