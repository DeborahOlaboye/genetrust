...
(define-public (is-valid-percentage-range (percentage uint))
  (ok (and
...
(define-public (is-positive-percentage (percentage uint))
  (ok (> percentage u0))
)

;; Guard: require percentage is valid
(define-public (guard-valid-percentage (percentage uint))
...
(define-public (guard-percentage-in-range (percentage uint))
...
(define-public (percentages-sum-to-hundred (p1 uint) (p2 uint) (p3 uint))
  (ok (is-eq (+ p1 (+ p2 p3)) u100))
)

;; Compare percentages
(define-public (compare-percentages (p1 uint) (p2 uint))
  (ok (if (< p1 p2) p2 p1))
)
